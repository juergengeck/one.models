...
export default class ChannelManager {
    ...
    public async postToChannel<T extends OneUnversionedObjectTypes>(
        ...
        try {
            ...
            if (waitForMergePromise !== undefined) {
                logWithId(channelId, owner, 'postToChannel - WAIT PROMISE');
                await waitForMergePromise;
                logWithId(channelId, owner, 'postToChannel - WAIT PROMISE DONE');
            }
        } catch (e) {
    ...
    public async getLatestMergedChannelInfoHash(
        ...
        const channelEntry = this.channelRegistry.channels.get(channelInfoIdHash);
    ...
    private async mergePendingVersions(
        ...
        try {
            ...
            await serializeWithType(`${cacheLockName}${channelInfoIdHash}`, async () => {
                // Load the cache & registry entry for the latest merged version
                const registryEntry = this.channelRegistry.channels.get(channelInfoIdHash);
                if (!registryEntry) {
                ...
                const cacheEntry = this.channelInfoCache.get(channelInfoIdHash);
                if (!cacheEntry) {
                    throw new Error('The channelInfoIdHash does not exist in channelInfoHash.');
                }
                ...
                if (commonHistoryHead) {
                    ...
                    if (rebuiltHead === channelInfosToMerge[channelInfosToMerge.length - 1].head) {
                        ...
                        registryEntry.readVersionIndex = lastVersionToMerge;
                        registryEntry.mergedVersionIndex = lastVersionToMerge;
                        ...
                        channelInfo.head = rebuiltHead;
                        const newVersion = await storeVersionedObject(channelInfo);
                        ...
                        if (newVersionIndex === lastVersionToMerge + 1) {
                            ...
                            registryEntry.readVersionIndex = newVersionIndex;
                            registryEntry.mergedVersionIndex = newVersionIndex;
        ...
        try {
            await serializeWithType(`${cacheLockName}${channelInfoIdHash}`, async () => {
                ...
                if (this.channelInfoCache.has(channelInfoIdHash)) {
                    ...
                    this.channelRegistry.channels.set(channelInfoIdHash, {
                        readVersionIndex: 0,
                        mergedVersionIndex: 0
                    });
                    await this.saveRegistryCacheToOne();
    ...
    private async saveRegistryCacheToOne(): Promise<void> {
        await serializeWithType(ChannelManager.registryLockName, async () => {
            const result = await storeVersionedObject(this.channelRegistry);

            // Note: If the write leads to a different value than the one passed in due to a
            // merge, we need to refresh the cache by calling the following line. It is commented,
            // because we don't have an easy  way to detect whether it is the same. We could
            // compare the read indexes of the old and new registries.
            // I did not implement this right now, because as long as the registry is not shared
            // in the IoM, this will not be necessary (sharing does not make sense right now).
            // this.channelInfoCache = await ChannelManager.computeChannelInfoCache(result.obj);
    ...
    private async loadRegistryCacheFromOne(): Promise<void> {
        ...
        await serializeWithType(ChannelManager.registryLockName, async () => {
            ...
            this.channelInfoCache = await ChannelManager.computeChannelInfoCache(registry);
            this.channelRegistry = registry;
        });
        logWithId(null, null, 'loadRegistryCacheFromOne - END');
    }

    private static async computeChannelInfoCache(
        registry: ChannelRegistry
    ): Promise<Map<SHA256IdHash<ChannelInfo>, ChannelInfoCacheEntry>> {
        const channelInfoCache: Map<SHA256IdHash<ChannelInfo>, ChannelInfoCacheEntry> = new Map();
        ...
        await Promise.all(
            [...registry.channels.entries()].map(async channel => {
                const [channelInfoIdHash, {readVersionIndex}] = channel;
                const channelInfoHash = await getNthVersionMapHash(
                    channelInfoIdHash,
                    readVersionIndex
                );
                channelInfoCache.set(channelInfoIdHash, {
