...
async function addVersionedObjToAccessibleObjectsMap(
    ...
    const versions = await getAllVersionMapEntries(idHash);

    for (const version of versions) {
        const accessibleObject = getOrCreate(accessibleObjects, version.hash, {
            type: 'versioned',
            hash: version.hash,
            idHash,
            oneType: idObj.$type$,
        ...
        if (accessibleObject.type === 'versioned') {
            accessibleObject.timestamps.push(version.timestamp);
        }
    }
}
